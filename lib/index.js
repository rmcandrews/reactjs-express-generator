...
var app, appName, description, userName, userEmail, userUrl;
...
var makeApp = function() {
  ...
  questions.askMany({
    ...
    appUrl: {info:'What is your apps url'},
    userName: {info:'What is your name'},
    userEmail: {info:'What is your email'},
    userUrl: {info:'What is your website url'}
  }, function(result){
    app = result;
    appName = app.appName;
    ...
    pkg = {
      ...
      "scripts": {
        ...
        "start": "concurrently --kill-others \"npm run react\" \"npm run styles\" \"npm run server\""
      }
    };
...
var createPackage = function(pkg) {
  fs.writeFile(currentDir() + '/package.json', JSON.stringify(pkg, null, 2), function (err) {
    ...
    createConfigFile(pkg);
  });
}

var createConfigFile = function(pkg) {
  ...
  fs.writeFile(currentDir() + '/config.js', cfg, function (err) {
    ...
    installNpmDependencies(pkg);
  });
}

var installNpmDependencies = function(pkg) {
